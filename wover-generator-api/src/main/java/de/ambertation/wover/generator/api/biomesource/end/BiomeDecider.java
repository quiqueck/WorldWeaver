...
import de.ambertation.wover.generator.impl.biomesource.end.BiomeDeciderImpl;
...
public abstract class BiomeDecider {
    ...
    /**
     * Seed-aware variant of {@link #createMap(BiomeMapBuilderFunction)}.
     * <p>
     * Called by the {@link WoverEndBiomeSource} whenever it (re)initializes its {@link BiomeMap}s for a
     * given world seed. The default implementation ignores {@code seed} and delegates to
     * {@link #createMap(BiomeMapBuilderFunction)} for backwards compatibility; override this if a decider
     * needs to derive additional per-seed state (e.g. its own noise fields) when the map is rebuilt.
     *
     * @param mapBuilder A function you can use to create a new {@link BiomeMap} that conforms to the settings
     *                   of the current BiomeSource.
     * @param seed       The world seed the map is being (re)built for.
     */
    ...
    /**
     * The tag whose biomes populate this decider's own {@link #picker() picker}.
     * <p>
     * If non-null, the {@link WoverEndBiomeSource} adds a picker entry for this tag (backed by this
     * decider's {@link #picker() picker}) <em>before</em> the standard center/barrens/void/land entries, so
     * that biomes carrying this tag are routed into the decider's picker instead of the default rings.
     *
     * @return the tag whose biomes feed this decider's picker, or {@code null} (the default) if this decider
     * contributes no own picker.
     */
    ...
    /**
     * This decider's own {@link WoverBiomePicker}, used together with {@link #pickerTag()} to route biomes
     * into a decider-managed picker.
     *
     * @return this decider's picker, or {@code null} if it has none.
     */
    public @Nullable WoverBiomePicker picker() {
        return picker;
    }
