...
import de.ambertation.wover.generator.api.chunkgenerator.ChunkGeneratorManager;
...
public class ChunkGeneratorManagerImpl {
    ...
    private static void onWorldCreation(
        ...
        // `access` is the registry provider the world's own WorldDimensions were resolved against -
        // see WorldGeneratorConfigImpl#writeWorldPresetSettings for why encoding needs this exact one
        // rather than the global WorldState.allStageRegistryAccess().
    ...
    public static void onWorldReCreate(
        ...
        for (var dimEntry : selectedDimensions.dimensions().entrySet()) {
