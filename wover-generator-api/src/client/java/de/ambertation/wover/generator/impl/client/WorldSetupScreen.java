...
import de.ambertation.wover.generator.api.client.biomesource.client.BiomeSourceConfigPanel;
import de.ambertation.wover.generator.api.client.biomesource.client.BiomeSourceWithConfigScreen;
...
import de.ambertation.wover.ui.impl.client.WelcomeScreen;
...
public class WorldSetupScreen extends LayoutScreen implements BiomeSourceConfigPanel.DimensionUpdater {
    ...
    private void generatorChanged(
        ...
        if (content.stack != null) {
            ...
            BiomeSourceConfigPanel<?, ?> panel = ClientBiomeSourceConfigScreenRegistry.createPanel(this, selectedBiomeSource);
            if (panel == null && selectedBiomeSource instanceof BiomeSourceWithConfigScreen<?, ?> bs) {
                // Legacy path for third-party BiomeSources that implement the interface directly instead
                // of registering with ClientBiomeSourceConfigScreenRegistry - only ever safe for classes
                // that are themselves client-only, since the interface's method touches client-only types.
                panel = bs.biomeSourceConfigPanel(this);
            }

            content.configPanel = panel;
            if (panel != null) {
                content.stack.replaceOrAdd(content.stackIndex, panel.getPanel());
            } else {
