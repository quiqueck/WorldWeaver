...
public class EndConfigPage implements BiomeSourceConfigPanel<WoverEndBiomeSource, WoverEndConfig> {
    ...
    public EndConfigPage(@NotNull WoverEndConfig endConfig) {
        ...
        caveBiomeSize = content.addRange(
                Value.fixed(200), Value.fit(),
                Component.translatable("title.screen.wover.worldgen.cave_biome_size"),
                1, 512, Math.max(1, endConfig.caveBiomesSize / 16)
        );
        ...
        caveTopY = content.addRange(
                Value.fixed(200), Value.fit(),
                Component.translatable("title.screen.wover.worldgen.cave_top_y"),
                0, 256, endConfig.caveBiomesTopY
        );

        caveTopJitter = content.addRange(
                Value.fixed(200), Value.fit(),
                Component.translatable("title.screen.wover.worldgen.cave_top_jitter"),
                0, 16, endConfig.caveBiomesTopJitter
        );
    ...
    public ChunkGenerator updateSettings(ChunkGenerator endGenerator) {
        if (endGenerator.getBiomeSource() instanceof BiomeSourceWithConfig<?, ?> bsc) {
            if (bsc.getBiomeSourceConfig() instanceof WoverEndConfig currentConfig) {
                ...
                WoverEndConfig endConfig = new WoverEndConfig(
                        ...
                        caveBiomeSize.getValue() * 16,
                        caveTopY.getValue(),
                        caveTopJitter.getValue()
                );
