...
import de.ambertation.wover.block.impl.trait.behaviour.LootTableTraitBuilder;
 ...
 * {@link de.ambertation.wover.block.api.BlockDefinition#addTrait} while the block is being configured, and the
...
public interface LootTableTrait extends BlockTrait<Block, LootTableTrait> {
    ...
    interface Builder extends BlockTraitBuilder.WithDefault<Block, LootTableTrait> {
        ...
        @Nullable LootTableTrait dropSelfNoExplosion();
         ...
         *
         * @param dropChance   the probability (<em>not</em> a 1-in-N denominator - a 1-in-16 drop is
         *                     {@code 1F / 16F}) that the sapling drops at fortune level 1; the no-fortune
         *                     chance is {@code 0.8 *} this, and fortune 2/3 scale it up to {@code 1.666 *}.
         *                     Vanilla leaves use {@code 0.0625F}. Must be in {@code [0, 0.6]}; anything
         *                     larger would push the fortune curve past 1.0 and drop a sapling every time.
         * @param saplingBlock the sapling dropped by this leaves block, or {@code null} if it drops none
         * @return the new trait, or {@code null} outside of a datagen environment
         */
