...
public class FindInDirection extends PlacementModifier {
    ...
    /**
     * The number of blocks that may be walked from {@code blockPos} towards {@code dir} without leaving the
     * chunk {@code blockPos} is in.
     * <p>
     * Vertical directions are unbounded - a chunk spans the full build height, so moving up or down can never
     * leave it.
     *
     * @param blockPos the position the search starts from
     * @param dir      the direction that is walked
     * @return the number of blocks available in {@code dir}, {@link Integer#MAX_VALUE} for {@code UP}/{@code DOWN}
     */
    private static int roomInChunk(BlockPos blockPos, Direction dir) {
        return switch (dir) {
            case EAST -> 15 - SectionPos.sectionRelative(blockPos.getX());   //+x
            case WEST -> SectionPos.sectionRelative(blockPos.getX());        //-x
            case SOUTH -> 15 - SectionPos.sectionRelative(blockPos.getZ());  //+z
            case NORTH -> SectionPos.sectionRelative(blockPos.getZ());       //-z
    ...
    private void submitSingle(
        ...
        // The probe must not leave the chunk that is currently being decorated. A placement modifier that
        // emits a position outside of it hands the feature an origin that is already out of bounds, and any
        // spread the feature applies on top of that (wover:random_patch's xz_spread, for example) then reads
        // terrain two chunks away - past minecraft:features' write radius of 1.
        //
        // The forward walk was already clamped this way; the backwards walk that runs when the start position
        // is inside the surface was not, and it is the one that escaped, because it travels against
        // searchDirection while being limited by the room available *along* it. Starting at the east edge of
        // a chunk and searching WEST, for example, gave it 15 blocks of budget to walk east, straight out of
        // the chunk.
        final int searchDist = Math.min(maxSearchDistance, roomInChunk(blockPos, searchDirection));
        final int oppositeDist = Math.min(
                maxSearchDistance,
                roomInChunk(blockPos, searchDirection.getOpposite())
        );
