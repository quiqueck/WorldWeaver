package de.ambertation.wover.ui.api;
...
public class VersionChecker implements Runnable {
     ...
     * <p>
     * The check only runs when {@code shouldCheck} is {@code true}, and the version checker was not already
     * started. If the last check is not yet stale (see {@link #WAIT_FOR_DAYS}), the cached response is reused
     * instead of contacting the update service again.
     * <p>
     * The client-only gate ({@code isClient}, the {@code check_for_new_versions} config option and whether the
     * welcome screen was already presented) is evaluated by the caller ({@code VersionCheckerClient}) and passed
     * in already resolved, since those checks depend on client-only config classes this common class must not
     * reference.
     *
     * @param shouldCheck {@code true} if every precondition for running the check is already met
     */
    public static void startCheck(boolean shouldCheck) {
        if (versionChecker == null && shouldCheck) {
