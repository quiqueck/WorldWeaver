...
public final class LootAdditionsImpl {
     ...
     */
    record Entry(ResourceLocation file, int order, LootAddition addition) {
    ...
    private static Map<ResourceKey<LootTable>, List<Entry>> load(
        ...
        final Map<ResourceLocation, List<Resource>> stacks = CONVERTER.listMatchingResourceStacks(resourceManager);
        // Sorted, so that the order two different files apply in is stable across runs and machines.
        final List<ResourceLocation> fileIds = new ArrayList<>(stacks.keySet());
        fileIds.sort(Comparator.comparing(ResourceLocation::toString));
        ...
        for (ResourceLocation fileId : fileIds) {
            final ResourceLocation name = CONVERTER.fileToId(fileId);
                ...
                for (ResourceKey<LootTable> target : addition.targets()) {
                    ...
                    if (!exists) {
                        ...
                        if (addition.required()) {
                            missingRequired.add(name + " -> " + target.location());
                        } else {
    ...
    private static LootAdditionFile decode(ResourceLocation name, Resource resource, RegistryOps<JsonElement> ops) {
