...
public class LootDatapackAdditionGameTest {
    ...
    public void datapackAdditionReachesARealContainer(GameTestHelper helper) {
        ...
        if (markers != 1) {
            failures.add("a chest filled from " + POOL_TABLE.location() + " held " + markers + " "
        ...
        if (total <= markers) {
            failures.add("a chest filled from " + POOL_TABLE.location() + " held nothing but the marker ("
        ...
        if (conditioned != null) {
            ...
            if (rolls[0] != ROLLS) {
                failures.add("expected all " + ROLLS + " rolls of " + CONDITION_TABLE.location()
            ...
            if (rolls[1] != ROLLS) {
                failures.add("expected every roll of " + CONDITION_TABLE.location() + " to include exactly"
        ...
        if (entryTable != null) {
            ...
            if (perPool.isEmpty()) {
                failures.add(ENTRY_TABLE.location() + " has no pools at all");
            } else if (perPool.stream().anyMatch(c -> c != 1)) {
                failures.add("expected exactly one " + ENTRY_MARKER + " entry in each pool of "
                        + ENTRY_TABLE.location() + " but found " + perPool + " (0 = add_entries with"
    ...
    public void untargetedAndGatedTablesStayClean(GameTestHelper helper) {
        ...
        if (countAll(gated) == 0) {
            failures.add("the resource-condition control chest is empty, so \"no marker\" proves nothing: "
                    + GATED_TABLE.location() + " never filled it");
        }
        final int gatedMarkers = count(gated, GATED_MARKER);
        if (gatedMarkers != 0) {
            failures.add("a chest filled from " + GATED_TABLE.location() + " held " + gatedMarkers + " "
    ...
    public void datapackAdditionsSurviveReapplication(GameTestHelper helper) {
        ...
        if (markersBefore != ROLLS) {
            failures.add("expected every one of " + ROLLS + " rolls of " + POOL_TABLE.location()
                    + " to yield exactly one " + POOL_MARKER + " but only " + markersBefore + " did");
        }
        if (markersAfter != markersBefore) {
            failures.add("re-applying changed how many rolls of " + POOL_TABLE.location() + " yield exactly"
    ...
    private static LootTable table(GameTestHelper helper, ResourceKey<LootTable> key, List<String> failures) {
        ...
        final LootTable table = reloadable
                ...
                .map(Holder::value)
                .orElse(null);
        if (table == null) failures.add(key.location() + " is missing from the loot registry");
        return table;
    }
