    ...
    public void conditionsGateExistingPoolsAndSurviveReapplication(GameTestHelper helper) {
        ...
        if (conditioned == null) {
            helper.fail(Component.literal(
                    TestModWoverLoot.CONDITIONED_TABLE.identifier() + " is missing from the loot registry"));
            return;
        }
        ...
        if (onlyMarker[0] != ROLLS) {
            failures.add("expected every one of " + ROLLS + " rolls of "
                    + TestModWoverLoot.CONDITIONED_TABLE.identifier() + " to yield only the marker (vanilla"
        ...
        if (onlyMarkerAfter[0] != ROLLS || onlyMarkerAfter[1] != ROLLS) {
            failures.add("re-applying the appenders changed " + TestModWoverLoot.CONDITIONED_TABLE.identifier()
        ...
        if (!conditionsAfter.equals(conditionsBefore)) {
            failures.add("re-applying the appenders changed the serialized pool conditions of "
                    + TestModWoverLoot.CONDITIONED_TABLE.identifier() + " (the restriction is accumulating"
